...
class SynthIDSparseTopKMixin(transformers.GenerationMixin):
  ...
  def _sample(
    ...
    while self._has_unfinished_sequences(  # pytype: disable=attribute-error
      ...
      values, indices = torch.topk(next_token_scores, k=top_k)
      
      # Save only top-k logits before watermarking
      with open("../data/logits_before_watermarking.json", "a") as f:
          json.dump({
              "next_token_scores": values.tolist(),
      ...
      if do_sample:
        *regular_warpers, watermarking_logits_warper = logits_warper
        ...
        # Save logits after watermarking (already top-k)
        with open("../data/logits_after_watermarking.json", "a") as f:
            json.dump({
                "next_token_scores": next_token_scores.tolist(),
                "next_token_indices": indices_mapping.tolist()
            }, f)
            f.write("\n")
      ...
      with open("../data/selected_tokens.json", "a") as f:
          json.dump({
